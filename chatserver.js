...
try {
  if (httpsOptions.key && httpsOptions.cert) {
    webServer = https.createServer(httpsOptions, (req, res) => {
      app.handle(req, res)
    });
  }
} catch(err) {
...
webServer.listen(6503, function() {
  log(`Server is listening on port https://${myHost}:6503`);
});
...
console.log('request.origin', request.origin);
  ...
  connection.on('close', function(reason, description) {
    ...
    // Build and output log output for close information. econnreset
